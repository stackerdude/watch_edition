...
3. Run ```./go pre-commit``` (OS X) or ```npm -s run test``` (Windows) or ```docker-compose -f docker-compose.test.yml up``` (Docker) to run the pre-commit checks - something should fail.
